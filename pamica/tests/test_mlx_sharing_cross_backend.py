...
1. the merge DECISION (MLX calls the same
   ...
   backend uses, on the same host float64 sensor maps ``pinv(sphere) @ A.T``,
   one column per component, issue #334); and
...
def test_merge_decision_matches_the_torch_backend():
    ...
    backend uses (issue #258) on host float64 ``pinv(sphere) @ A.T``, which is
    the metric ``AMICATorchNG._identify_shared_comps`` computes inline. Driving both
    ...
    # Perturb one cross-model component (a row, issue #334) into NEAR- (not
    # exact-) collinearity, so the decision itself is under test rather than a
    # pre-merged state.
    a_np = np.array(model.A)
...
def test_gm_prev_weighting_matches_torch(merged):
    ...
    ``merged=False`` is the disjoint-``comp_list`` case: every component has a
    ...
    weighting is observable at all: a shared component row averages two models'
    steps with those weights. Measured on this state (component-row layout,
    issue #334): 6.3e-8 with the fix against 5.6e-3 with the post-update ``gm``
    restored -- five orders of magnitude, which is what the tolerance below
    discriminates on.
    """
    ...
    # 1e-4: 56x below the post-update-gm failure (5.6e-3) and three orders above
    # the float32-vs-float64 agreement this pair actually reaches (6.3e-8).
