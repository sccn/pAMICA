...
these predicates with its own counters (``.rules/backend_parity.md``).
"""
...
def newton_active(do_newton: bool, index: int, newt_start: int) -> bool:
    ...
    amica15.f90:1498/1651/1719/1739/1804).

    ``newt_start`` counts from 1, as the reference's does: ``newt_start=1``
    (or ``0``) takes the Newton step from the first iteration, ``newt_start=50``
    from the 50th.
    """
